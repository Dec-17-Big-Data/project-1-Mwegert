 ...
 * global population with higher education that is unemployed. The data is
 * averaged by POPULATION rather than by country.</h4>
 * 
 * <p>In order to accomplish this, four fields need to be extracted from
 * the database. These are 1) % female unemployment with advanced education, 
 * 2) total female population, 3) % male unemployment with advanced education,
 * and 4) total population.
 * 
 * <h5>The purpose of each MapReduce job is as follows:</h5>
 * 
 * <p>MapReducer 1: Convert these 4 fields into two: a flat number of
 * unemployed people, and a flat number of total population to add to the pool.
 * Male and Female fields are combined with a reduce side join.</p>
